import './index.scss'
import React,{Component} from 'react'
import GroupState from "../../../commons/Group"
import LoginUserForm from "./LoginUserForm"
import LoginTextForm from "./LoginTextForm"
class textLogin extends Component{
    constructor(){
        super();
        //this.login = this.login.bind(this)
        this.state = {
            loginType:"user"
        }
    }
    // login(){
    //     //这个方法用于更改redux里面的userInfo状态
    //     this.props.login({
    //         username:"123",
    //         password:"456",
    //         success:data=>{
    //             console.log(data)
    //             //this.props.history.replace("/mine/user")
    //         },
    //         fail:err=>{
    //             console.log(err)
    //         }
    //     })
    // }
    //切换表单
    changeLoginType(){
        let Form = LoginUserForm;
        let {loginType} = this.state;
        let title = "手机号快捷登录"
        let type = "text"
        if(loginType !== "user"){
            Form = LoginTextForm;
            title="账号密码登录"
            type = "user"
        }
        const ChangeType=()=>{
            this.setState({
                loginType:type
            })
        }
        return(
           <div className="login-common">
                <Form />
                <div className="change">
                    <a href="script:;" onClick={ChangeType}>{title}></a>
                </div>
           </div>
        )
    }
    render(){
        return(
            <div className="login">
                <div className="icon">
                    <img alt="" src="data:image/png;base64,"/> 
                    <div className="arrow"><i className="fa fa-chevron-left"></i></div>
                </div>
                <div className="login-content">
                    {this.changeLoginType()}
                </div>
            </div>
        )
    }
}
export default GroupState(textLogin,{
    reducer:"mine",
    states:["userInfo"]
})